...
set -e
cd /d/Code/blessing-skin-server-cf/pages
...
echo "=== 4. smoke-e2e ==="
SMOKE_BASE=http://127.0.0.1:$PORT SMOKE_STATE=$STATE node scripts/smoke-e2e.mjs > /tmp/final-e2e.txt 2>&1 || true
grep -cE "✓" /tmp/final-e2e.txt || true
grep -cE "✗" /tmp/final-e2e.txt || true
grep -E "全部通过|失败" /tmp/final-e2e.txt | tail -2 || true

echo "=== 5. oauth-test ==="
SMOKE_BASE=http://127.0.0.1:$PORT SMOKE_STATE=$STATE node scripts/oauth-test.mjs > /tmp/final-oauth.txt 2>&1 || true
