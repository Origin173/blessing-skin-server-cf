...
| 阶段 | 内容 | 出口标准 | 状态 |
|---|---|---|---|
| **P0 脚手架** | `pages/` 初始化：OpenNext + wrangler.toml（复用绑定）+ D1 migrations + 会话/CSRF + 插件运行时骨架（事件分发/Filter/PluginManager/注册表）+ 布局（Layout RSC 对应 base.twig + adminlte 样式）+ 语言包 | `next build` 通过；`/` 与 `/auth/login` 可访问；插件 SDK 冒烟（示例插件注入 header） | ✅ 完成 (2026-08-10) |
...
- P5：与原版功能清单逐项对照（附对照表验收记录）；legacy（pages/functions + pages/web）已删除
