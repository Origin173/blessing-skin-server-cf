...
> 本文档是 `pages/next/` 新项目的实现依据；现有 `pages/functions/`（Hono）+ `pages/web/`（Vite SPA）
> 保留作对照，功能对齐后移除。
...
├── migrations/                # 复用现有 D1 migrations (软链或拷贝)
├── src/
...
| `pages/functions/[[path]].ts` + `pages/web` | 保留在 `pages/legacy/` 作对照，对齐后删除 |
